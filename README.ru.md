...
- напоминание о промывке по количеству шотов и автоматическая программа backflush
...
### Напоминание и программа промывки группы
...
`Silvia Backflush Reminder Shots` задаёт порог напоминания. По умолчанию это `60` шотов, значение настраивается из Home Assistant. Значение `0` отключает напоминание. Когда счётчик доходит до порога, `Silvia Backflush Status` показывает, что пора промыть группу. Это только напоминание, оно не блокирует приготовление кофе.

`Silvia Start Backflush` запускает автоматическую программу промывки помпой и трёхходовым клапаном. `Silvia Stop Backflush` прерывает запущенную программу и выключает помпу с клапаном. `Silvia Reset Backflush Shots` вручную сбрасывает счётчик напоминания. После успешного завершения программы счётчик промывки сбрасывается автоматически.
