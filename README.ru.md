...
- опциональный стартовый пинок помпы и его длительность;
...
`Silvia Pump Profile` выбирает open-loop кривую мощности помпы для кастомного выхода `ac_cycle_skip`. Это команда мощности, а не цель давления: без датчика давления контроллер не знает фактическое давление и не может удерживать его в замкнутом контуре.

Текущие профили помпы:

- `Classic`: обычная работа помпы на 100%;
- `Lever`: плавная кривая `35% -> 75% -> 45%`;
- `Slayer Style`: плавный низкопоточный старт `30% -> 55%`, затем удержание;
