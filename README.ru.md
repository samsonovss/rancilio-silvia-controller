...
- двухзонный разогрев без накопления интегратора вдали от цели;
- усреднение производной PID для подавления шума PT100;
- ограничение интегратора и настраиваемый диапазон подхода к цели;
...
2. Скопируйте `esphome/rancilio-silvia-power.yaml`, `esphome/shot_profiles.h`, `esphome/shot_diagnostics.h` и `esphome/components/ac_cycle_skip/` в каталог конфигурации ESPHome.
...
- коэффициенты PID;
- диапазон освобождения интегратора при разогреве;
- профиль пролива;
...
### PID без большого перелёта при первом нагреве

Обычный PID одинаково работает и около рабочей температуры, и при разгоне холодного бойлера. На длинном разгоне его интегральная составляющая успевает накопить большую положительную ошибку. Когда датчик достигает цели, регулятор продолжает подавать мощность, а запасённое в ТЭНе и металле тепло дополнительно разгоняет температуру.

В контроллере используется двухзонная логика:

```text
далеко от цели → PID работает, но интегратор удерживается на нуле
в диапазоне подхода → освобождается полный PID
около цели → полный PID удерживает температуру
```

PID не отключается во время разогрева. Пропорциональная и производная составляющие продолжают управлять нагревателем и заранее снижать мощность. Замораживается только интегратор, пока измеренная температура ниже цели более чем на `Silvia PID Warmup Range`. По умолчанию диапазон равен `10 °C`. После входа в него интегратор начинает работать, но ограничен диапазоном `0…20%`.

Основные отличия от обычного PID:

- защита от integral windup именно на длинном разогреве;
- ограниченный рабочий интегратор вместо возможности долго требовать высокую мощность;
- производная усредняется по пяти измерениям PT100;
- выход PID не усредняется, поэтому торможение нагревателя не получает дополнительной задержки;
- границу подхода можно менять из Home Assistant;
- состояние стартового режима и P/I/D-составляющие доступны отдельными диагностическими сенсорами.

Текущие проверенные начальные коэффициенты:
...
Контрольный тест на реальной Rancilio Silvia при цели `93 °C` и старте с `52,8 °C` показал максимум `93,79 °C`: перелёт составил `+0,79 °C`. До введения стартовой защиты при сопоставимом старте максимум достигал `99,58 °C`, то есть перелёт составлял `+6,58 °C`.

Эти коэффициенты и температурная поправка зависят от расположения датчика, конкретного бойлера и сборки. Значения следует считать проверенной исходной настройкой этого прототипа, а не универсальной калибровкой для любой Silvia.
