...
- `Custom`: редактируемые времена фаз и кривая `Custom Start/Main/End Pump Power`.

Ручное изменение времени, custom-мощности фазы или phase boost автоматически переводит профиль в `Custom`. `Silvia Manual Pump Power` отделён от рецепта и используется только для ручного включения помпы, горячей воды и обслуживания.
...
Автоматические профили делают snapshot выбранного рецепта на старте шота, обновляют рассчитанную мощность примерно раз в `200 мс` и временно ставят `ac_cycle_skip.ramp_ms = 0`: плавность формирует сам shot profile через smoothstep. Изменения в Home Assistant во время пролива применятся к следующему шоту, а не к текущему. После остановки или отмены пролива пользовательские `Silvia Pump Ramp Time` и ручной boost восстанавливаются.

Это всё ещё команда мощности, а не цель давления: без датчика давления контроллер не знает фактическое давление и не может удерживать его в замкнутом контуре. Текущая реализация работает только как open-loop `POWER`; для версии с датчиком давления сценарий нужно будет вынести в явные фазы `POWER`/`PRESSURE`.
