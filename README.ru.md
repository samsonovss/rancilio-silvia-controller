...
5. выключить помпу, выполнить импульс реле brew-клапана `OFF -> ON` на одну
   секунду для сброса остаточного давления, затем вернуть `OFF`, чтобы катушка
   не оставалась под напряжением.
