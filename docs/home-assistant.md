...
Автоматический шот делает snapshot параметров на старте, включая `Silvia Pump Start Boost Time`, и разворачивает рецепт в массив `ShotPhase`: изменения в Home Assistant во время пролива применятся только к следующему шоту. Сейчас исполняются только open-loop `POWER`-фазы; режимы `PRESSURE` и `FLOW` зарезервированы для будущих датчиков. Автоматические профили временно используют `ac_cycle_skip` без дополнительного ramp, потому что плавность уже рассчитана внутри профиля. Ручной `Silvia Pump Ramp Time` применяется к обычному ручному включению помпы, горячей воде и backflush и восстанавливается после остановки автоматического шота.

Phase boost означает разрешение применить snapshot-значение `Silvia Pump Start Boost Time`, если эта фаза начинается с остановленной помпы. Непрерывный переход из предсмачивания прямо в основной пролив не создаёт второй принудительный пинок на `100%`; для повторного boost нужна пауза/выключение помпы, как в Bloom.
