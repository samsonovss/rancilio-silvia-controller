...
void IRAM_ATTR HOT ACCycleSkipDataStore::schedule_gate_pulse_() {
  if (this->requested_q16.load(std::memory_order_relaxed) == 0) {
    this->force_off_();
    return;
  }

  this->gate_pin.digital_write(false);
  esp_rom_delay_us(this->gate_delay_us.load(std::memory_order_relaxed));
  if (this->requested_q16.load(std::memory_order_relaxed) == 0) {
    this->force_off_();
    return;
  }
  this->gate_pin.digital_write(true);
  esp_rom_delay_us(this->gate_pulse_us.load(std::memory_order_relaxed));
  this->gate_pin.digital_write(false);
}
...
void ACCycleSkipOutput::write_state(float state) {
  ...
  if (starting && start_boost_ms > 0) {
    ...
    this->store_.boost_until_us.store(now + start_boost_ms * 1000UL, std::memory_order_relaxed);
  }
}
